...
for w_id, name in workflow_ids.items():
    print(f"Checking workflow: {name} ({w_id})...")
    # Fetch
    ...
    for node in wf["nodes"]:
        if node["name"] == "Send a message" and "guildId" in node["parameters"]:
            del node["parameters"]["guildId"]
            modified = True
            print("  Removed guildId parameter.")
